...
function QBCore.Functions.LookAtEntity(entity, timeout, speed)
	...
	while true do
		...
		Wait(0)
		if (startTimeout + timeout) < GetGameTimer() then
			break
		end
	end
...
function QBCore.Functions.LoadParticleDictionary(dictionary)
	if HasNamedPtfxAssetLoaded(dictionary) then
		return
	end
...
function QBCore.Functions.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
	...
	if looped then
		particleHandle =
			StartParticleFxLoopedAtCoord(ptName, coords.x, coords.y, coords.z, rot.x, rot.y, rot.z, scale or 1.0)
		if color then
...
function QBCore.Functions.StartParticleOnEntity(
	dict,
	ptName,
	looped,
	entity,
	...
	if looped then
		if bone then
			...
			particleHandle =
				StartParticleFxLoopedOnEntity(ptName, entity, offset.x, offset.y, offset.z, rot.x, rot.y, rot.z, scale)
		end
		if evolution then
...
function QBCore.Functions.GetGroundHash(entity)
	local coords = GetEntityCoords(entity)
	local num =
		StartShapeTestCapsule(coords.x, coords.y, coords.z + 4, coords.x, coords.y, coords.z - 2.0, 1, 1, entity, 7)
