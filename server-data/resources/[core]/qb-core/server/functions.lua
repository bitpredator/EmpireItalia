...
function PaycheckInterval()
	...
	for _, Player in pairs(QBCore.Players) do
		...
		local payment =
			QBShared.Jobs[Player.PlayerData.job.name]["grades"][tostring(Player.PlayerData.job.grade.level)].payment
		if not payment then
		...
		if
			...
			and payment > 0
			and (QBShared.Jobs[Player.PlayerData.job.name].offDutyPay or Player.PlayerData.job.onduty)
		then
...
function QBCore.Functions.IsWhitelisted(source)
	if not QBCore.Config.Server.Whitelist then
		return true
	end
	if QBCore.Functions.HasPermission(source, QBCore.Config.Server.WhitelistPermission) then
		return true
	end
	return false
end
...
function QBCore.Functions.HasPermission(source, permission)
	...
	elseif type(permission) == "table" then
		for _, permLevel in pairs(permission) do
			if IsPlayerAceAllowed(source, permLevel) then
				return true
			end
		end
	end
...
function QBCore.Functions.IsOptin(source)
	...
	if not license or not QBCore.Functions.HasPermission(source, "admin") then
		return false
	end
