...
function QBCore.Commands.Add(name, help, arguments, argsrequired, callback, permission, ...)
	...
	if not permission then
		permission = "user"
	end -- some commands don't pass permission level
	if permission == "user" then
		restricted = false
	end -- allow all users to use command
	...
	function(source, args)
		if args[1] and not args[2] and not args[3] then
			...
			if args[1] and args[2] and args[3] then
				local x = tonumber((args[1]:gsub(",", ""))) + 0.0
				local y = tonumber((args[2]:gsub(",", ""))) + 0.0
				local z = tonumber((args[3]:gsub(",", ""))) + 0.0
				local heading = args[4] and tonumber((args[4]:gsub(",", ""))) + 0.0 or false
