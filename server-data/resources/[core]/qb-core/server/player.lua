...
function QBCore.Player.CheckPlayerData(source, PlayerData)
	...
	if
		PlayerData.gang
		and PlayerData.gang.name ~= nil
		and PlayerData.gang.grade
		and PlayerData.gang.grade.level ~= nil
	then
...
function QBCore.Player.CreateCitizenId()
	...
	local result =
		MySQL.prepare.await("SELECT EXISTS(SELECT 1 FROM players WHERE citizenid = ?) AS uniqueCheck", { CitizenId })
	if result == 0 then
		return CitizenId
	end
