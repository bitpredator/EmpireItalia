	...
	if Player then
		...
		SetTimeout(200, function()
			local mails = MySQL.query.await(
				"SELECT * FROM player_mails WHERE citizenid = ? ORDER BY `date` ASC",
				{ Player.PlayerData.citizenid }
			)
			if mails[1] ~= nil then
...
QBCore.Functions.CreateCallback("qb-phone:server:GetInvoices", function(source, cb)
	...
	if Player then
		local invoices =
			MySQL.query.await("SELECT * FROM phone_invoices WHERE citizenid = ?", { Player.PlayerData.citizenid })
...
QBCore.Functions.CreateCallback("qb-phone:server:GetPhoneData", function(source, cb)
	...
	if Player ~= nil then
		...
		local result = MySQL.query.await(
			"SELECT * FROM player_contacts WHERE citizenid = ? ORDER BY name ASC",
			{ Player.PlayerData.citizenid }
		)
		...
		local garageresult =
			MySQL.query.await("SELECT * FROM player_vehicles WHERE citizenid = ?", { Player.PlayerData.citizenid })
		if garageresult[1] ~= nil then
			PhoneData.Garage = garageresult
		end

		local messages =
			MySQL.query.await("SELECT * FROM phone_messages WHERE citizenid = ?", { Player.PlayerData.citizenid })
		...
		local Tweets = MySQL.query.await(
			"SELECT * FROM phone_tweets WHERE `date` > NOW() - INTERVAL ? hour",
		...
		local transactions = MySQL.query.await(
			"SELECT * FROM crypto_transactions WHERE citizenid = ? ORDER BY `date` ASC",
			{ Player.PlayerData.citizenid }
		)
		...
		local images = MySQL.query.await(
			"SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC",
			{ Player.PlayerData.citizenid }
		)
	...
	function(source, cb, society, amount, invoiceId, sendercitizenid)
		...
		if Ply then
			local exists = MySQL.query.await(
				"select count(1) as count FROM phone_invoices WHERE id = ? and citizenid = ?",
				{ invoiceId, Ply.PlayerData.citizenid }
			)
			...
			if exists[1] and exists[1]["count"] == 1 then
				if SenderPly and Config.BillingCommissions[society] then
					...
					invoiceMailData = {
						...
						message = string.format(
							...
							commission,
							Ply.PlayerData.charinfo.firstname,
							Ply.PlayerData.charinfo.lastname,
							amount
						),
					}
				...
				if Ply.Functions.RemoveMoney("bank", amount, "paid-invoice") then
					MySQL.query(
						"DELETE FROM phone_invoices WHERE id = ? and citizenid = ?",
						{ invoiceId, Ply.PlayerData.citizenid }
					)
...
QBCore.Functions.CreateCallback("qb-phone:server:DeclineInvoice", function(source, cb, _, _, invoiceId)
	...
	if Ply then
		local exists = MySQL.query.await(
			"select count(1) as count FROM phone_invoices WHERE id = ? and citizenid = ? and candecline = ?",
			{ invoiceId, Ply.PlayerData.citizenid, 1 }
		)

		if exists[1] and exists[1]["count"] == 1 then
			TriggerEvent("qb-phone:server:declinedInvoice", source, invoiceId)
			MySQL.query(
				"DELETE FROM phone_invoices WHERE id = ? and citizenid = ? and candecline = ?",
				{ invoiceId, Ply.PlayerData.citizenid, 1 }
			)
			cb(true)
			return
...
QBCore.Functions.CreateCallback("qb-phone:server:GetVehicleSearchResults", function(_, cb, search)
	...
	local query = "%" .. search .. "%"
	local result =
		MySQL.query.await("SELECT * FROM player_vehicles WHERE plate LIKE ? OR citizenid = ?", { query, search })
	if result[1] ~= nil then
...
QBCore.Functions.CreateCallback("qb-phone:server:GetCurrentLawyers", function(_, cb)
	...
	for _, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if Player ~= nil then
			if
				(
					Player.PlayerData.job.name == "lawyer"
					or Player.PlayerData.job.name == "realestate"
					or Player.PlayerData.job.name == "mechanic"
					or Player.PlayerData.job.name == "taxi"
					or Player.PlayerData.job.name == "police"
					or Player.PlayerData.job.name == "ambulance"
...
QBCore.Functions.CreateCallback("qb-phone:server:GetWebhook", function(_, cb)
	if WebHook ~= "" then
		...
		print(
			"Set your webhook to ensure that your camera will work!!!!!! Set this on line 9 of the server sided script!!!!!"
		)
		cb(nil)
	end
end)
...
RegisterNetEvent("qb-phone:server:sendNewMail", function(mailData)
	...
	SetTimeout(200, function()
		local mails = MySQL.query.await(
			"SELECT * FROM player_mails WHERE citizenid = ? ORDER BY `date` DESC",
			{ Player.PlayerData.citizenid }
		)
		if mails[1] ~= nil then
			for k, _ in pairs(mails) do
...
RegisterNetEvent("qb-phone:server:sendNewEventMail", function(citizenid, mailData)
	...
	SetTimeout(200, function()
		local mails =
			MySQL.query.await("SELECT * FROM player_mails WHERE citizenid = ? ORDER BY `date` ASC", { citizenid })
		if mails[1] ~= nil then
			for k, _ in pairs(mails) do
				if mails[k].button ~= nil then
					mails[k].button = json.decode(mails[k].button)
...
RegisterNetEvent("qb-phone:server:ClearButtonData", function(mailId)
	...
	local Player = QBCore.Functions.GetPlayer(src)
	MySQL.update(
		"UPDATE player_mails SET button = ? WHERE mailid = ? AND citizenid = ?",
	...
	for _, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if Player ~= nil then
			if
				...
				and Player.PlayerData.charinfo.lastname == lastName
			then
				...
				local result = MySQL.query.await(
					"SELECT * FROM players WHERE charinfo LIKE ? AND charinfo LIKE ?",
					{ query1, query2 }
				)
				if result[1] ~= nil then
...
RegisterNetEvent("qb-phone:server:BillingEmail", function(data, paid)
	for _, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		if target.PlayerData.job.name == data.society then
			if paid then
				local name = ""
					.. QBCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname
					.. " "
...
RegisterNetEvent("qb-phone:server:RemoveContact", function(Name, Number)
	...
	local Player = QBCore.Functions.GetPlayer(src)
	MySQL.query(
		"DELETE FROM player_contacts WHERE name = ? AND number = ? AND citizenid = ?",
...
RegisterNetEvent("qb-phone:server:UpdateMessages", function(ChatMessages, ChatNumber, _)
	...
	if Player[1] ~= nil then
		...
		if TargetData ~= nil then
			local Chat = MySQL.query.await(
				"SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?",
...
QBCore.Commands.Add(
	...
	function(source, args)
		...
		if
			biller.PlayerData.job.name == "police"
			or biller.PlayerData.job.name == "ambulance"
			or biller.PlayerData.job.name == "mechanic"
		then
			if billed ~= nil then
